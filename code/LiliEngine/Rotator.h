...
public:
	...
	FORCEINLINE static uint8_t CompressAxisToByte(float angle);

	FORCEINLINE static float DecompressAxisFromByte(uint8_t angle);

	FORCEINLINE static uint16_t CompressAxisToShort(float angle);

	FORCEINLINE static float DecompressAxisFromShort(uint16_t angle);
...
{
	return (math::Abs(NormalizeAxis(pitch - rhs.pitch)) <= tolerance)
		&& (math::Abs(NormalizeAxis(yaw - rhs.yaw)) <= tolerance)
		&& (math::Abs(NormalizeAxis(roll - rhs.roll)) <= tolerance);
}
...
FORCEINLINE uint8_t Rotator::CompressAxisToByte(float angle)
{
	return math::RoundToInt(angle * 256.f / 360.f) & 0xFF;
}

FORCEINLINE float Rotator::DecompressAxisFromByte(uint8_t angle)
{
	return (angle * 360.f / 256.f);
}

FORCEINLINE uint16_t Rotator::CompressAxisToShort(float angle)
{
	return math::RoundToInt(angle * 65536.f / 360.f) & 0xFFFF;
}

FORCEINLINE float Rotator::DecompressAxisFromShort(uint16_t angle)
{
...
// inline functions
template<class U>
FORCEINLINE Rotator math::Lerp(const Rotator& a, const Rotator& b, const U& alpha)
{
	return a + (b - a).GetNormalized() * alpha;
}

template<class U>
FORCEINLINE Rotator math::LerpRange(const Rotator& a, const Rotator& b, const U& alpha)
{
