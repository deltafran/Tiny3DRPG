...
public:
	...
	FORCEINLINE void CopyColumnFrom(int32_t column, const Vector4& vec);

	FORCEINLINE void CopyColumnTo(int32_t column, Vector4& vec) const;
	...
	FORCEINLINE void CopyRawFrom(int32_t column, const Vector4& vec);

	FORCEINLINE void CopyRawTo(int32_t column, Vector4& vec) const;
	...
	FORCEINLINE void SetAxis(int32_t i, const Vector3& axis);
	...
	FORCEINLINE Vector3 GetColumn(int32_t col) const;
...
{
	for (uint32_t RowIndex = 0; (RowIndex < NumRows) && (RowIndex < 4); RowIndex++)
	{
		for (uint32_t ColumnIndex = 0; (ColumnIndex < NumColumns) && (ColumnIndex < 4); ColumnIndex++) {
...
FORCEINLINE void Matrix4x4::PrependRotation(const Rotator& rotator, const Vector3& pivot)
{
	...
	math::VectorMatrixMultiply(this, &rotMatrix, this);
}

FORCEINLINE void Matrix4x4::AppendRotation(const Rotator& rotator, const Vector3& pivot)
{
	Matrix4x4 rotMatrix(rotator, pivot);
	math::VectorMatrixMultiply(this, this, &rotMatrix);
}
...
FORCEINLINE void Matrix4x4::CopyColumnFrom(int32_t column, const Vector4& vec)
{
	m[0][column] = vec.x;
	m[1][column] = vec.y;
	m[2][column] = vec.z;
...
FORCEINLINE void Matrix4x4::CopyColumnTo(int32_t column, Vector4& vec) const
{
	vec.x = m[0][column];
	vec.y = m[1][column];
	vec.z = m[2][column];
...
FORCEINLINE void Matrix4x4::CopyRawFrom(int32_t raw, const Vector4& vec)
{
	m[raw][0] = vec.x;
	m[raw][1] = vec.y;
	m[raw][2] = vec.z;
...
FORCEINLINE void Matrix4x4::CopyRawTo(int32_t raw, Vector4& vec) const
{
	vec.x = m[raw][0];
	vec.y = m[raw][1];
	vec.z = m[raw][2];
...
FORCEINLINE void Matrix4x4::SetAxis(int32_t i, const Vector3& axis)
{
	m[i][0] = axis.x;
	m[i][1] = axis.y;
	m[i][2] = axis.z;
...
FORCEINLINE Vector3 Matrix4x4::GetColumn(int32_t i) const
{
