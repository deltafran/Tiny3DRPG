...
// -------------- VKRenderTargetLayout --------------
VKRenderTargetLayout::VKRenderTargetLayout(const VKRenderPassInfo& renderPassInfo)
{
    ...
    for (int32_t index = 0; index < renderPassInfo.numColorRenderTargets; ++index)
    {
        const VKRenderPassInfo::ColorEntry& colorEntry = renderPassInfo.colorRenderTargets[index];
        VKTexture* texture = colorEntry.renderTarget;
    ...
    {
        VKTexture* texture = renderPassInfo.depthStencilRenderTarget.depthStencilTarget;
        VkAttachmentDescription& attchmentDescription = descriptions[numAttachmentDescriptions];
...
uint16_t VKRenderTargetLayout::SetupSubpasses(VkSubpassDescription* outDescs, uint32_t maxDescs, VkSubpassDependency* outDeps, uint32_t maxDeps, uint32_t& outNumDependencies) const
{
...
// -------------- VKRenderPass --------------
VKRenderPass::VKRenderPass(VkDevice inDevice, const VKRenderTargetLayout& rtLayout)
    : layout(rtLayout)
    , device(inDevice)
{
    ...
    uint16_t numSubpasses = rtLayout.SetupSubpasses(subpassDesc, 1, subpassDep, 1, numDependencies);
    ...
    {
        const uint32_t viewMask = (1 << rtLayout.extent3D.depth) - 1;
        const uint32_t correlationMask = (1 << rtLayout.extent3D.depth) - 1;
...
// -------------- VKFrameBuffer --------------
VKFrameBuffer::VKFrameBuffer(VkDevice inDevice, const VKRenderTargetLayout& rtLayout, const VKRenderPass& renderPass, const VKRenderPassInfo& renderPassInfo)
    : device(inDevice)
{
    ...
    for (int32_t index = 0; index < renderPassInfo.numColorRenderTargets; ++index)
    {
        VKTexture* texture = renderPassInfo.colorRenderTargets[index].renderTarget;
...
void VKRenderTarget::BeginRenderPass(VkCommandBuffer commandBuffer)
{
    for (int32_t index = 0; index < renderPassInfo.numColorRenderTargets; ++index)
    {
        VKTexture* texture = renderPassInfo.colorRenderTargets[index].renderTarget;
        ...
        vkutils::ImagePipelineBarrier(commandBuffer, image, ImageLayoutBarrier::Undefined, ImageLayoutBarrier::ColorAttachment, subResRange);
    }
    ...
    if (renderPassInfo.depthStencilRenderTarget.depthStencilTarget)
    {
        ...
        subResRange.baseArrayLayer = 0;
        vkutils::ImagePipelineBarrier(commandBuffer, image, ImageLayoutBarrier::Undefined, ImageLayoutBarrier::DepthStencilAttachment, subResRange);
    }
...
void VKRenderTarget::EndRenderPass(VkCommandBuffer commandBuffer)
{
    ...
    for (int32_t index = 0; index < renderPassInfo.numColorRenderTargets; ++index)
    {
        ...
        subResRange.layerCount = texture->depth;
        subResRange.baseArrayLayer = 0;
        vkutils::ImagePipelineBarrier(commandBuffer, image, ImageLayoutBarrier::ColorAttachment, colorLayout, subResRange);
    }
    ...
    if (renderPassInfo.depthStencilRenderTarget.depthStencilTarget)
    {
        ...
        subResRange.layerCount = renderPassInfo.depthStencilRenderTarget.depthStencilTarget->depth;
        subResRange.baseArrayLayer = 0;
        vkutils::ImagePipelineBarrier(commandBuffer, image, ImageLayoutBarrier::DepthStencilAttachment, depthLayout, subResRange);
    }
}

VKRenderTarget* VKRenderTarget::Create(std::shared_ptr<VulkanDevice> vulkanDevice, const VKRenderPassInfo& inRenderPassInfo)
{
    VkDevice device = vulkanDevice->GetInstanceHandle();

    VKRenderTarget* renderTarget = new VKRenderTarget(inRenderPassInfo);
    renderTarget->device = device;
    renderTarget->renderPass = new VKRenderPass(device, renderTarget->rtLayout);
    renderTarget->frameBuffer = new VKFrameBuffer(device, renderTarget->rtLayout, *(renderTarget->renderPass), inRenderPassInfo);
    renderTarget->extent2D = renderTarget->frameBuffer->extent2D;

    return renderTarget;
}

VKRenderTarget* VKRenderTarget::Create(std::shared_ptr<VulkanDevice> vulkanDevice, const VKRenderPassInfo& inRenderPassInfo, Vector4 clearColor)
{
    VkDevice device = vulkanDevice->GetInstanceHandle();

    VKRenderTarget* renderTarget = new VKRenderTarget(inRenderPassInfo, clearColor);
    renderTarget->device = device;
    renderTarget->renderPass = new VKRenderPass(device, renderTarget->rtLayout);
    renderTarget->frameBuffer = new VKFrameBuffer(device, renderTarget->rtLayout, *(renderTarget->renderPass), inRenderPassInfo);
    renderTarget->extent2D = renderTarget->frameBuffer->extent2D;
