...
void ImageGUIContext::Init(const std::string& font, VulkanDeviceRef vulkanDevice, float windowWidth, float windowHeight, float frameWidth, float frameHeight)
{
	ImGui::CreateContext();
...
void ImageGUIContext::StartFrame()
{
	...
	ImGuiIO& io = ImGui::GetIO();
	io.MouseWheel += InputSystem::GetMouseDelta();
	io.MousePos = ImVec2(mousePos.x * io.DisplayFramebufferScale.x, mousePos.y * io.DisplayFramebufferScale.y);
	io.MouseDown[0] = InputSystem::IsMouseDown(MouseType::MOUSE_BUTTON_LEFT);
	io.MouseDown[1] = InputSystem::IsMouseDown(MouseType::MOUSE_BUTTON_RIGHT);
	io.MouseDown[2] = InputSystem::IsMouseDown(MouseType::MOUSE_BUTTON_MIDDLE);

	io.KeyCtrl = InputSystem::IsKeyDown(KeyboardType::KEY_LEFT_CONTROL) || InputSystem::IsKeyDown(KeyboardType::KEY_RIGHT_CONTROL);
	io.KeyShift = InputSystem::IsKeyDown(KeyboardType::KEY_LEFT_SHIFT) || InputSystem::IsKeyDown(KeyboardType::KEY_RIGHT_SHIFT);
	io.KeyAlt = InputSystem::IsKeyDown(KeyboardType::KEY_LEFT_ALT) || InputSystem::IsKeyDown(KeyboardType::KEY_RIGHT_ALT);
	io.KeySuper = false;
