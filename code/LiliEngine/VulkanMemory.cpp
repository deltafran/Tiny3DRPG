...
constexpr uint32_t VulkanResourceHeapManager::m_PoolSizes[(int32_t)VulkanResourceHeapManager::PoolSizes::SizesCount];
constexpr uint32_t VulkanResourceHeapManager::m_BufferSizes[(int32_t)VulkanResourceHeapManager::PoolSizes::SizesCount + 1];
...
void VulkanRange::JoinConsecutiveRanges(std::vector<VulkanRange>& ranges)
{
    ...
    for (int32_t index = (int32_t)ranges.size() - 1; index > 0; --index)
    {
...
{
    for (int32_t index = 0; index < m_HeapInfos.size(); ++index)
    {
...
VulkanDeviceMemoryAllocation* VulkanDeviceMemoryManager::Alloc(bool canFail, VkDeviceSize allocationSize, uint32_t memoryTypeIndex, void* dedicatedAllocateInfo, const char* file, uint32_t line)
{
    ...
    m_NumAllocations += 1;
    m_PeakNumAllocations = std::max(m_NumAllocations, m_PeakNumAllocations);
    ...
    uint32_t heapIndex = m_MemoryProperties.memoryTypes[memoryTypeIndex].heapIndex;
    m_HeapInfos[heapIndex].allocations.push_back(newAllocation);
    m_HeapInfos[heapIndex].usedSize += allocationSize;
    m_HeapInfos[heapIndex].peakSize = std::max(m_HeapInfos[heapIndex].peakSize, m_HeapInfos[heapIndex].usedSize);
...
void VulkanDeviceMemoryManager::Free(VulkanDeviceMemoryAllocation*& allocation)
{
    ...
    uint32_t heapIndex = m_MemoryProperties.memoryTypes[allocation->m_MemoryTypeIndex].heapIndex;
...
void VulkanDeviceMemoryManager::DumpMemory()
{
    ...
    for (int32_t index = 0; index < m_HeapInfos.size(); ++index)
    {
        ...
        for (int32_t subIndex = 0; subIndex < heapInfo.allocations.size(); ++subIndex)
        {
            VulkanDeviceMemoryAllocation* allocation = heapInfo.allocations[subIndex];
            MLOG("\t\t%d Size %llu Handle %p", subIndex, (uint64_t)allocation->m_Size, (void*)allocation->m_Handle);
...
uint64_t VulkanDeviceMemoryManager::GetTotalMemory(bool gpu) const
{
    uint64_t totalMemory = 0;
    for (uint32_t index = 0; index < m_MemoryProperties.memoryHeapCount; ++index)
    {
...
{
    const uint32_t maxAllocations = m_Device->GetLimits().maxMemoryAllocationCount;
    ...
    for (uint32_t index = 0; index < m_MemoryProperties.memoryHeapCount; ++index)
    {
        ...
        MLOG(
            ...
            (uint64_t)(m_MemoryProperties.memoryHeaps[index].size),
    ...
    for (uint32_t index = 0; index < m_MemoryProperties.memoryHeapCount; ++index)
    {
        bool isGPUHeap = ((m_MemoryProperties.memoryHeaps[index].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) == VK_MEMORY_HEAP_DEVICE_LOCAL_BIT);
        if (isGPUHeap) {
            m_HeapInfos[index].totalSize = (uint64_t)((float)m_HeapInfos[index].totalSize * 0.95f);
        }
    }
}

// VulkanResourceAllocation
VulkanResourceAllocation::VulkanResourceAllocation(VulkanResourceHeapPage* owner, VulkanDeviceMemoryAllocation* deviceMemoryAllocation, uint32_t requestedSize, uint32_t alignedOffset, uint32_t allocationSize, uint32_t allocationOffset, const char* file, uint32_t line)
    : m_Owner(owner)
...
VulkanResourceHeapPage::VulkanResourceHeapPage(VulkanResourceHeap* owner, VulkanDeviceMemoryAllocation* deviceMemoryAllocation, uint32_t id)
    : m_Owner(owner)
    , m_DeviceMemoryAllocation(deviceMemoryAllocation)
    , m_MaxSize(0)
    , m_UsedSize(0)
    ...
    , m_ID(id)
{
    m_MaxSize = (uint32_t)m_DeviceMemoryAllocation->GetSize();
...
VulkanResourceAllocation* VulkanResourceHeapPage::TryAllocate(uint32_t size, uint32_t alignment, const char* file, uint32_t line)
{
    for (int32_t index = 0; index < m_FreeList.size(); ++index)
    {
        VulkanRange& entry = m_FreeList[index];
        uint32_t allocatedOffset = entry.offset;
        uint32_t alignedOffset = Align(entry.offset, alignment);
        uint32_t alignmentAdjustment = alignedOffset - entry.offset;
        uint32_t allocatedSize = alignmentAdjustment + size;
        ...
        if (allocatedSize <= entry.size)
        {
            ...
            m_PeakNumAllocations = std::max((uint32_t)m_PeakNumAllocations, (uint32_t)m_ResourceAllocations.size());
...
bool VulkanResourceHeapPage::JoinFreeBlocks()
{
    ...
    if (m_FreeList.size() == 1)
    {
        ...
        {
            if (m_UsedSize > 0) {
                MLOGE("Memory leak, used size = %d", (int32_t)m_UsedSize);
            }
...
VulkanResourceHeap::VulkanResourceHeap(VulkanResourceHeapManager* owner, uint32_t memoryTypeIndex, uint32_t pageSize)
    : m_Owner(owner)
...
VulkanResourceHeap::~VulkanResourceHeap()
{
    ...
    {
        bool leak = false;
        for (int32_t index = (int32_t)usedPages.size() - 1; index >= 0; --index)
        {
...
void VulkanResourceHeap::FreePage(VulkanResourceHeapPage* page)
{
    ...
    bool removed = false;
    for (int32_t i = 0; i < m_UsedBufferPages.size(); ++i)
    {
    ...
    for (int32_t i = 0; i < m_UsedImagePages.size(); ++i)
    {
...
{
    for (int32_t index = 0; index < m_FreePages.size(); ++index)
    {
...
{
    MLOG("%d Free Pages", (int32_t)m_FreePages.size());

    auto DumpPages = [&](std::vector<VulkanResourceHeapPage*>& usedPages, const char* typeName)
    {
        MLOG("\t%s Pages: %d Used, Peak Allocation Size on a Page %d", typeName, (int32_t)usedPages.size(), m_PeakPageSize);
        uint64_t subAllocUsedMemory = 0;
        uint64_t subAllocAllocatedMemory = 0;
        uint32_t numSubAllocations = 0;
        for (int32_t index = 0; index < usedPages.size(); ++index)
        {
            subAllocUsedMemory += usedPages[index]->m_UsedSize;
            subAllocAllocatedMemory += usedPages[index]->m_MaxSize;
            numSubAllocations += (uint32_t)usedPages[index]->m_ResourceAllocations.size();
            MLOG("\t\t%d: ID %4d %4d suballocs, %4d free chunks (%d used/%d free/%d max) DeviceMemory %p", index, usedPages[index]->GetID(), (int32_t)usedPages[index]->m_ResourceAllocations.size(), (int32_t)usedPages[index]->m_FreeList.size(), usedPages[index]->m_UsedSize, usedPages[index]->m_MaxSize - usedPages[index]->m_UsedSize, usedPages[index]->m_MaxSize, (void*)usedPages[index]->m_DeviceMemoryAllocation->GetHandle());
        }

        MLOG("%d Suballocations for Used/Total: %d/%d = %.2f%%", numSubAllocations, (int32_t)subAllocUsedMemory, (int32_t)subAllocAllocatedMemory, subAllocAllocatedMemory > 0 ? 100.0f * (float)subAllocUsedMemory / (float)subAllocAllocatedMemory : 0.0f);
    };
...
VulkanResourceAllocation* VulkanResourceHeap::AllocateResource(Type type, uint32_t size, uint32_t alignment, bool mapAllocation, const char* file, uint32_t line)
{
    std::vector<VulkanResourceHeapPage*>& usedPages = type == Type::Image ? m_UsedImagePages : m_UsedBufferPages;
    uint32_t targetDefaultPageSize = m_DefaultPageSize;
    ...
    uint32_t allocationSize = std::max(size, targetDefaultPageSize);
    ...
    m_PeakPageSize = std::max(m_PeakPageSize, allocationSize);
...
VulkanResourceSubAllocation::VulkanResourceSubAllocation(uint32_t requestedSize, uint32_t alignedOffset, uint32_t allocationSize, uint32_t allocationOffset)
    : m_RequestedSize(requestedSize)
    , m_AlignedOffset(alignedOffset)
    , m_AllocationSize(allocationSize)
    , m_AllocationOffset(allocationOffset)
...
VulkanBufferSubAllocation::VulkanBufferSubAllocation(VulkanSubBufferAllocator* owner, VkBuffer handle, uint32_t requestedSize, uint32_t alignedOffset, uint32_t allocationSize, uint32_t allocationOffset)
...
{
    return (uint8_t*)m_Owner->GetMappedPointer() + m_AlignedOffset;
}

// VulkanSubResourceAllocator
VulkanSubResourceAllocator::VulkanSubResourceAllocator(VulkanResourceHeapManager* owner, VulkanDeviceMemoryAllocation* deviceMemoryAllocation, uint32_t memoryTypeIndex, VkMemoryPropertyFlags memoryPropertyFlags, uint32_t alignment)
    : m_Owner(owner)
    , m_MemoryTypeIndex(memoryTypeIndex)
    ...
    , m_UsedSize(0)
{
    m_MaxSize = (uint32_t)deviceMemoryAllocation->GetSize();
...
VulkanResourceSubAllocation* VulkanSubResourceAllocator::TryAllocateNoLocking(uint32_t size, uint32_t alignment, const char* file, uint32_t line)
{
    ...
    for (int32_t index = 0; index < m_FreeList.size(); ++index)
    {
        VulkanRange& entry = m_FreeList[index];
        uint32_t allocatedOffset = entry.offset;
        uint32_t alignedOffset = Align(entry.offset, m_Alignment);
        uint32_t alignmentAdjustment = alignedOffset - entry.offset;
        uint32_t allocatedSize = alignmentAdjustment + size;
...
VulkanSubBufferAllocator::VulkanSubBufferAllocator(VulkanResourceHeapManager* owner, VulkanDeviceMemoryAllocation* deviceMemoryAllocation, uint32_t memoryTypeIndex, VkMemoryPropertyFlags memoryPropertyFlags, uint32_t alignment, VkBuffer buffer, VkBufferUsageFlags bufferUsageFlags, int32_t poolSizeIndex)
...
VulkanResourceSubAllocation* VulkanSubBufferAllocator::CreateSubAllocation(uint32_t size, uint32_t alignedOffset, uint32_t allocatedSize, uint32_t allocatedOffset)
{
...
{
    bool released = false;
    for (int32_t index = 0; index < m_SubAllocations.size(); ++index)
    {
...
void VulkanResourceHeapManager::Init()
{
    ...
    const uint32_t typeBits = (1 << memoryManager.GetNumMemoryTypes()) - 1;
    ...
    auto GetMemoryTypesFromProperties = [memoryProperties](uint32_t typeBits, VkMemoryPropertyFlags properties, std::vector<uint32_t>& outTypeIndices)
    {
        for (uint32_t i = 0; i < memoryProperties.memoryTypeCount && typeBits; ++i)
        {
            if ((typeBits & 1) == 1)
            {
        ...
        for (int32_t index = (int32_t)outTypeIndices.size() - 1; index >= 1; --index)
        {
...

    {
        ...
        for (int32_t index = 0; index < typeIndices.size(); ++index)
        {
            int32_t heapIndex = memoryProperties.memoryTypes[typeIndices[index]].heapIndex;
            VkDeviceSize heapSize = memoryProperties.memoryHeaps[heapIndex].size;
            VkDeviceSize pageSize = std::min<VkDeviceSize>(heapSize / 8, GPU_ONLY_HEAP_PAGE_SIZE);
            m_ResourceTypeHeaps[typeIndices[index]] = new VulkanResourceHeap(this, typeIndices[index], uint32_t(pageSize));
...
void VulkanResourceHeapManager::Destory()
{
    ...
    for (int32_t index = 0; index < m_ResourceTypeHeaps.size(); ++index)
    {
...
VulkanResourceAllocation* VulkanResourceHeapManager::AllocateBufferMemory(const VkMemoryRequirements& memoryReqs, VkMemoryPropertyFlags memoryPropertyFlags, const char* file, uint32_t line)
{
    uint32_t typeIndex = 0;
    ...
    if (!m_ResourceTypeHeaps[typeIndex])
    {
        ...
        {
            MLOG("Unable to find alternate type for index %d, MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %s(%d)", originalTypeIndex, (uint32_t)memoryReqs.size, (uint32_t)memoryReqs.memoryTypeBits, (uint32_t)memoryPropertyFlags, file, line);
        }

#ifdef _DEBUG
        DumpMemory();
#endif
        MLOG("Missing memory type index %d (originally requested %d), MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %s(%d)", typeIndex, originalTypeIndex, (uint32_t)memoryReqs.size, (uint32_t)memoryReqs.memoryTypeBits, (uint32_t)memoryPropertyFlags, file, line);
    }

    VulkanResourceAllocation* allocation = m_ResourceTypeHeaps[typeIndex]->AllocateResource(VulkanResourceHeap::Type::Buffer, uint32_t(memoryReqs.size), uint32_t(memoryReqs.alignment), canMapped, file, line);
    ...
    if (!allocation)
    {
        ...
        if (!m_ResourceTypeHeaps[typeIndex]) {
            MLOG("Missing memory type index %d, MemSize %d, MemPropTypeBits %u, MemPropertyFlags %u, %s(%d)", typeIndex, (uint32_t)memoryReqs.size, (uint32_t)memoryReqs.memoryTypeBits, (uint32_t)memoryPropertyFlags, file, line);
        }
        allocation = m_ResourceTypeHeaps[typeIndex]->AllocateResource(VulkanResourceHeap::Type::Buffer, uint32_t(memoryReqs.size), uint32_t(memoryReqs.alignment), canMapped, file, line);
    }

    return allocation;
}

VulkanResourceAllocation* VulkanResourceHeapManager::AllocateImageMemory(const VkMemoryRequirements& memoryReqs, VkMemoryPropertyFlags memoryPropertyFlags, const char* file, uint32_t line)
{
    uint32_t typeIndex = 0;
    VERIFYVULKANRESULT(m_DeviceMemoryManager->GetMemoryTypeFromProperties(memoryReqs.memoryTypeBits, memoryPropertyFlags, &typeIndex));
    ...
    VulkanResourceAllocation* allocation = m_ResourceTypeHeaps[typeIndex]->AllocateResource(VulkanResourceHeap::Type::Image, uint32_t(memoryReqs.size), uint32_t(memoryReqs.alignment), canMapped, file, line);

    if (!allocation)
    {
        VERIFYVULKANRESULT(m_DeviceMemoryManager->GetMemoryTypeFromPropertiesExcluding(memoryReqs.memoryTypeBits, memoryPropertyFlags, typeIndex, &typeIndex));
        canMapped = (memoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) == VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
        allocation = m_ResourceTypeHeaps[typeIndex]->AllocateResource(VulkanResourceHeap::Type::Image, uint32_t(memoryReqs.size), uint32_t(memoryReqs.alignment), canMapped, file, line);
    }

    return allocation;
}

VulkanBufferSubAllocation* VulkanResourceHeapManager::AllocateBuffer(uint32_t size, VkBufferUsageFlags bufferUsageFlags, VkMemoryPropertyFlags memoryPropertyFlags, const char* file, uint32_t line)
{
    ...
    if (isStorageOrTexel)
    {
        alignment = std::max(alignment, ((bufferUsageFlags & (VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT)) != 0) ? (uint32_t)limits.minTexelBufferOffsetAlignment : 1u);
        alignment = std::max(alignment, ((bufferUsageFlags & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT) != 0) ? (uint32_t)limits.minStorageBufferOffsetAlignment : 1u);
    }
    else
    {
        alignment = (uint32_t)limits.minUniformBufferOffsetAlignment;
    ...
    int32_t poolSize = (int32_t)GetPoolTypeForAlloc(size, alignment);
    if (poolSize != (int32_t)PoolSizes::SizesCount) {
        size = m_PoolSizes[poolSize];
    }

    for (int32_t index = 0; index < m_UsedBufferAllocations[poolSize].size(); ++index)
    {
    ...
    for (int32_t index = 0; index < m_FreeBufferAllocations[poolSize].size(); ++index)
    {
    ...
    uint32_t bufferSize = std::max(size, m_BufferSizes[poolSize]);
    ...
    alignment = std::max((uint32_t)memReqs.alignment, alignment);
    ...
    VulkanSubBufferAllocator* bufferAllocation = new VulkanSubBufferAllocator(this, deviceMemoryAllocation, memoryTypeIndex, memoryPropertyFlags, uint32_t(memReqs.alignment), buffer, bufferUsageFlags, poolSize);
    m_UsedBufferAllocations[poolSize].push_back(bufferAllocation);
...
void VulkanResourceHeapManager::ReleaseBuffer(VulkanSubBufferAllocator* bufferAllocator)
{
    ...
    for (int32_t index = 0; index < m_UsedBufferAllocations[bufferAllocator->m_PoolSizeIndex].size(); ++index)
    {
...
void VulkanResourceHeapManager::DumpMemory()
{
    ...
    uint64_t usedBinnedTotal = 0;
    uint64_t allocBinnedTotal = 0;
    uint64_t usedLargeTotal = 0;
    uint64_t allocLargeTotal = 0;

    for (int32_t poolSizeIndex = 0; poolSizeIndex < (int32_t)PoolSizes::SizesCount + 1; poolSizeIndex++)
    {
        ...
        if (poolSizeIndex == (int32_t)PoolSizes::SizesCount) {
            MLOG("Buffer of large size Allocations: %d Used / %d Free", (int32_t)usedAllocations.size(), (int32_t)freeAllocations.size());
        }
        else {
            MLOG("Buffer of %d size Allocations: %d Used / %d Free", m_PoolSizes[poolSizeIndex], (int32_t)usedAllocations.size(), (int32_t)freeAllocations.size());
        }

        if (usedAllocations.size() > 0)
        {
            uint64_t _UsedBinnedTotal = 0;
            uint64_t _AllocBinnedTotal = 0;
            uint64_t _UsedLargeTotal = 0;
            uint64_t _AllocLargeTotal = 0;

            MLOG("Index  BufferHandle   DeviceMemoryHandle MemFlags BufferFlags #Suballocs #FreeChunks UsedSize/MaxSize");
            for (int32_t index = 0; index < usedAllocations.size(); ++index)
            {
                VulkanSubBufferAllocator* bufferAllocation = usedAllocations[index];
                MLOG("%6d %p %p 0x%06x 0x%08x %6d   %6d    %d/%d", index, (void*)bufferAllocation->m_Buffer, (void*)bufferAllocation->m_DeviceMemoryAllocation->GetHandle(), bufferAllocation->m_MemoryPropertyFlags, bufferAllocation->m_BufferUsageFlags, (int32_t)bufferAllocation->m_SubAllocations.size(), (int32_t)bufferAllocation->m_FreeList.size(), (int32_t)bufferAllocation->m_UsedSize, bufferAllocation->m_MaxSize);
            ...
            if (poolSizeIndex == (int32_t)PoolSizes::SizesCount) {
                MLOG(" Large Alloc Used/Max %d/%d %.2f%%", (int32_t)_UsedLargeTotal, (int32_t)_AllocLargeTotal, 100.0f * (float)_UsedLargeTotal / (float)_AllocLargeTotal);
            }
            else {
                MLOG(" Binned [%d] Alloc Used/Max %d/%d %.2f%%", m_PoolSizes[poolSizeIndex], (int32_t)_UsedBinnedTotal, (int32_t)_AllocBinnedTotal, 100.0f * (float)_UsedBinnedTotal / (float)_AllocBinnedTotal);
            }
        }
    }

    MLOG("::Totals::");
    MLOG("Large Alloc Used/Max %d/%d %.2f%%", (int32_t)usedLargeTotal, (int32_t)allocLargeTotal, 100.0f * allocLargeTotal > 0 ? (float)usedLargeTotal / (float)allocLargeTotal : 0.0f);
    MLOG("Binned Alloc Used/Max %d/%d %.2f%%", (int32_t)usedBinnedTotal, (int32_t)allocBinnedTotal, allocBinnedTotal > 0 ? 100.0f * (float)usedBinnedTotal / (float)allocBinnedTotal : 0.0f);
}
#endif
...
void VulkanResourceHeapManager::ReleaseFreedResources(bool immediately)
{
    ...
    {
        for (int32_t index = 0; index < freeAllocations.size(); ++index)
        {
...
void VulkanResourceHeapManager::DestroyResourceAllocations()
{
    ...
    {
        for (int32_t index = (int32_t)usedAllocations.size() - 1; index >= 0; --index)
        {
            VulkanSubBufferAllocator* bufferAllocation = usedAllocations[index];
