...
{
public:
	DynamicUniformBuffer(Configuration& configuration) noexcept;

	void StartGame() noexcept;
private:
	DynamicUniformBuffer() = delete;
	DynamicUniformBuffer(const DynamicUniformBuffer&) = delete;
	DynamicUniformBuffer(DynamicUniformBuffer&&) = delete;
	DynamicUniformBuffer operator=(const DynamicUniformBuffer&) = delete;
	DynamicUniformBuffer operator=(DynamicUniformBuffer&&) = delete;
	...
	bool UpdateUI(float time, float delta)
	{
...

		{
			...
			uint32_t colorAlign = Align(sizeof(ColorBlock), alignment);
	...
	void SetupCommandBuffers()
	{
		...
		for (int32_t i = 0; i < m_defContext->m_CommandBuffers.size(); ++i)
		{
			...
			for (int32_t meshIndex = 0; meshIndex < m_Model->meshes.size(); ++meshIndex)
			{
				...
				vkCmdBindDescriptorSets(m_defContext->m_CommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, m_Pipeline->pipelineLayout, 0, 1, &m_DescriptorSet, 2, dynamicOffsets);
	...
	void CreatePipelines()
	{
		...
		pipelineInfo.vertShaderModule = vkutils::LoadSPIPVShader(m_Device, "data/shaders/9_DynamicUniformBuffer/obj.vert.spv");
		pipelineInfo.fragShaderModule = vkutils::LoadSPIPVShader(m_Device, "data/shaders/9_DynamicUniformBuffer/obj.frag.spv");
	...
	void CreateUniformBuffers()
	{
		...
		m_ColorBuffer = VKBuffer::CreateBuffer(
			m_VulkanDevice,
			VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
			m_ColorDatas.size(),
	...
	std::vector<uint8_t>			    m_ColorDatas;
	VKBuffer* m_ColorBuffer = nullptr;
